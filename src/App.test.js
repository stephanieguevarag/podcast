...
import App from "./App";
import { PodcastContext } from "./ui/context/Podcast.context";
import { getPodcast } from "./ui/views/Home/Home.controller";
import { getPodcastDetail } from "./ui/views/PodcastDetailView/PodcastDetailView.controller";
...
const mockCurrentEpisode = {
  title: 'Episode 580 | "Ghost Decorer"',
  releaseDate: "23/11/2022",
  duration: "19:54",
  episodeUrl:
    "https://traffic.libsyn.com/secure/jbpod/Joe_Budden_Podcast_580.mp3?dest-id=2422538",
  description:
    "The JBP kicks things off with a hip-hop debate that pits Ish & Ice vs. Joe & Parks. The guys then talk about what they do when refurnishing their homes and what areas they struggle with (20:50). In recent news, the AMA’s canceled Chris Brown’s Michael Jackson Thriller tribute (36:20), Meek Mill reveals he didn’t know how much money he made off of “Going Bad” with Drake despite his label saying the song made $24 million (1:03:57). Latto had 130 unreleased songs leak (1:22:30), Part of the Show returns (1:41:25), + MORE!\n Become a Patron of The Joe Budden Podcast for additional bonus episodes and visual content for all things JBP.: Tap in here www.patreon.com/JoeBudden\n  \n Sleeper Picks:\n Joe | Samaria - “Stuck”\n Ice | Gucci Mane - “Letter To Takeoff”\n Parks | Ab-Soul - “Hollandaise”\n Ish | SABRI - “Broken Promises”",
  id: 1000587230252,
};
...
jest.mock("./ui/views/Home/Home.controller.ts");

const mockedGetPodcast = mocked(getPodcast, true);

jest.mock("./ui/views/PodcastDetailView/PodcastDetailView.controller.ts");
...
describe("App view - Podcast list", () => {
  beforeEach(() => {
  ...
  test("Should show first podcast item", async () => {
    renderComponent();
    const firstPodcastItem = await screen.findByTestId("podcast-item-0");
    await waitFor(async () => {
      expect(firstPodcastItem).toBeInTheDocument();
    });
  });
});

describe("App view - Podcast selected", () => {
  beforeEach(() => {
    mockedGetPodcast.mockReturnValue(Promise.resolve(mockResponse));
  });

  test("Should show podcastDetail with episode list when podcast is selected", async () => {
    renderComponent();
    const firstPodcastItem = await screen.findByTestId("podcast-item-0");
    await waitFor(async () => {
      firstPodcastItem.click();
      mockedGetPodcastDetail.mockReturnValue(
        Promise.resolve(mockDetailResponse)
      );
    });
    const podcastDetail = await screen.findByTestId("podcast-detail");
    const episodeList = await screen.findByTestId("episode-list");
    expect(podcastDetail).toBeInTheDocument();
    expect(episodeList).toBeInTheDocument();
  });

  test("Should show episode detail when episode is selected", async () => {
    renderComponent();
    const firstPodcastItem = await screen.findByTestId("podcast-item-0");
    await waitFor(async () => {
      firstPodcastItem.click();
    ...
    const episodeLink = await screen.findByTestId("table-link-1");
    expect(episodeLink).toBeInTheDocument();
    await waitFor(async () => {
      episodeLink.click();
    });

    const currentEpisode = await screen.findByTestId("current-episode");
    expect(currentEpisode).toBeInTheDocument();
  });
});
